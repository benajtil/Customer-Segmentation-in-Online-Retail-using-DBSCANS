...
<body>
    ...
    <script>
        // Global variable to store rules after loading
var globalRules = null;

// Load the rules JSON file on page load
function loadRules() {
  fetch('includes/json/insights.json')
    .then(response => response.json())
    .then(data => { 
      globalRules = data; 
      console.log("Rules loaded:", globalRules);
    })
    .catch(err => console.error('Error loading rules:', err));
}
loadRules();

// Utility to check a single condition
function checkCondition(detailValue, operator, conditionValue, details) {
  detailValue = parseFloat(detailValue);
  conditionValue = parseFloat(conditionValue);

  // Custom operators for average order value
  if (operator === 'avgLT') {
    if (details.Frequency && parseFloat(details.Frequency) > 0) {
      let avg = parseFloat(details.Monetary) / parseFloat(details.Frequency);
      return avg < conditionValue;
    }
    return false;
  } else if (operator === 'avgGT') {
    if (details.Frequency && parseFloat(details.Frequency) > 0) {
      let avg = parseFloat(details.Monetary) / parseFloat(details.Frequency);
      return avg > conditionValue;
    }
    ...
    switch (operator) {
      case '>=': return detailValue >= conditionValue;
      case '<=': return detailValue <= conditionValue;
      case '>':  return detailValue > conditionValue;
      case '<':  return detailValue < conditionValue;
      case '==': return detailValue === conditionValue;
...
function checkConditions(details, conditions) {
  return conditions.every(function(cond) {
    return checkCondition(details[cond.field], cond.operator, cond.value, details);
  });
}

// Generates KMeans insights based on the dynamic JSON rules
function generateKMeansInsights(details, segmentation) {
  if (!globalRules) {
    return { insights: "No rules loaded.", suggestions: "No suggestions available." };
  }
  var rules = globalRules[segmentation] || [];
  var insights = "";
  var suggestions = "";
  
  rules.forEach(function(rule) {
    if (checkConditions(details, rule.conditions)) {
      insights += rule.insight + "\n";
      suggestions += rule.suggestion + "\n";
    }
  });
  
  if (insights.trim() === "") {
    insights = "No special patterns detected. Check overall cluster or gather more data.";
  }
  if (suggestions.trim() === "") {
    suggestions = "Maintain regular communication and track metric changes over time.";
  }
